...
import { NetworkRestriction as NetworkRestrictionType, Platform } from '@/types/models';
...
export function NetworkRestriction({ platform, profileId, initialData, onSave, onCancel }: NetworkRestrictionProps) {
    ...
    const [formData, setFormData] = useState<Partial<NetworkRestrictionType>>({
        allowWifi: initialData?.allowWifi ?? true,
        ...initialData
    });
    ...
    const renderView = () => (
        <div className="space-y-6 max-w-4xl mt-6">
            <div className="flex items-center justify-between pb-4 border-b">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Wi-Fi controls</p>
                    </div>
                </div>
            ...
            <Card className={`border-l-4 ${formData.allowWifi ? 'border-l-green-500' : 'border-l-red-500'}`}>
                <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                        <Wifi className="w-5 h-5 text-green-500" />
                        <span className="font-medium">Wi-Fi</span>
                    </div>
                    <Badge variant={formData.allowWifi ? 'default' : 'destructive'}>
                        {formData.allowWifi ? 'Allowed' : 'Blocked'}
                    </Badge>
                </CardContent>
            </Card>
            ...
            <div className="flex items-center justify-between pb-4 border-b">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Configure Wi-Fi policy</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-4 border rounded-xl bg-card">
                <div className="flex items-center justify-between">
                    <Label className="flex items-start gap-3">
                        <Wifi className="w-5 h-5 mt-0.5 text-green-500" />
                        <div>
                            <span className="font-medium">Allow Wi-Fi</span>
                    ...
                    <Switch
                        checked={formData.allowWifi}
                        onCheckedChange={(c) => setFormData(prev => ({ ...prev, allowWifi: c }))}
                    />
                </div>
            </div>
