...
import { Battery, Edit, Loader2, Monitor, Navigation, Power, Save, Settings, Shield, Square, Trash2 } from 'lucide-react';
...
export function KioskRestriction({ platform, profileId, initialData, onSave, onCancel, hideFooter, formId }: KioskRestrictionProps) {
    const { t } = useLanguage();
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await restrictionAPI.deleteKioskRestriction(platform, profileId);
            toast({ title: 'Success', description: 'Kiosk restriction removed.' });
            onSave();
        } catch (error) {
            toast({ title: 'Error', description: getErrorMessage(error, 'Failed to remove kiosk restriction'), variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    };
